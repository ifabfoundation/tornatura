...
function DetectionStepTreatment({ formData, onNextClick, action }: DetectionProps) {
  ...
  const [isAddingTreatment, setIsAddingTreatment] = React.useState(() =>
    hasSavedTreatment
      ? savedTreatments.length === 0 && formData.detectionData.treatment.treatment
      : false,
  );
...
export function DetectionForm() {
  ...
  const handleNextClick = async (data: DetectionStepData) => {
    ...
    if (currentStepKey === "treatment") {
      const treatmentData = data as DetectionStepTreatmentData;
      setFormData((prev) => ({
        ...prev,
        detectionData: {
          ...prev.detectionData,
