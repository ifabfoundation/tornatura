...
import { BollettinoResponse, fetchCultureReport } from "../../../services/model-api";
...
export function FieldModelBollettini() {
  ...
  React.useEffect(() => {
    dispatch(
      headerbarActions.setTitle({
        title: `Bollettino: ${formatHarvestName(currentField?.harvest)}`,
        subtitle: "Bollettini fitosanitari",
      }),
    );
  }, [currentField?.harvest, dispatch]);

  React.useEffect(() => {
    if (!currentField?.harvest) {
      setReport(null);
      setError("La coltura del campo non è disponibile.");
      setLoading(false);
      return;
    }
    ...
    setLoading(true);
    setError(null);
    fetchCultureReport(currentField.harvest, lat, lng)
      .then((data) => {
        setReport(data);
      })
      .catch((err) => {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Non è stato possibile caricare il bollettino del campo.",
        );
      })
      .finally(() => {
        setLoading(false);
      });
  }, [centroid, currentField?.harvest]);

  const title = `Bollettino fitosanitario: ${formatHarvestName(currentField?.harvest)}`;
              ...
              <section className="soft bg-white">
                <div id="model-markdown">
                  <ReactMarkdown remarkPlugins={[remarkGfm]}>
                    {report?.content ?? "Nessun contenuto disponibile per questo bollettino."}
                  </ReactMarkdown>
                </div>
              </section>
            </Fragment>
